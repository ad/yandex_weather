...
ATTR_WIND_BEARING_TXT = 'wind_bearing_txt'
ATTR_WEATHER_CON_EMOJI = 'weather_condition_emoji'
...
WIND_BEARINGS = {
    ...
    'sw': 'Юго-западный',
    'w': 'Западный',
    'c': 'Практически отсутствует'
}

WEATHER_CONDITIONS_EMOJI = {
    'clear': '\U00002600',
...
class YandexWeather(WeatherEntity):
    ...
    def __init__(self, name: str, longitude: str, latitude: str, api_key: str, loop, session):
        ...
        self._weather_data = YaWeather(self._latitude, self._longitude, self._api_key, self._hloop,
                                       session=self._hsession)
    ...
    def forecast(self):
        ...
        if self._weather_data.forecast is not None:
            ...
            for data_in in fc_array:
                ...
                data_out[ATTR_FORECAST_WIND_SPEED] = (round(data_in.get('wind_speed') * 18 / 5)
                                                      if 'wind_speed' in data_in else None)
                ...
                data_out[ATTR_WEATHER_CON_EMOJI] = WEATHER_CONDITIONS_EMOJI[data_in.get('condition')]
                data_out[ATTR_WIND_BEARING_TXT] = WIND_BEARINGS[data_in.get('wind_dir')]
    ...
    def device_state_attributes(self):
        ...
        if self._weather_data.current is not None:
            ...
            data[ATTR_OBS_TIME] = dt_util.as_local(
                dt_util.utc_from_timestamp(self._weather_data.current.get('obs_time'))).strftime(TIME_STR_FORMAT)
            data[ATTR_WEATHER_CON] = DESCRIPTION_DIC[self._weather_data.current.get('condition')]
            data[ATTR_WEATHER_CON_EMOJI] = WEATHER_CONDITIONS_EMOJI[self._weather_data.current.get('condition')]
            data[ATTR_WIND_BEARING_TXT] = WIND_BEARINGS[self._weather_data.current.get('wind_dir')]
            return data
        return None
